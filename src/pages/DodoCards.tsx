...
export default function DodoCards() {
  ...
  return (
    <div className="page-enter">
      <div className="row" style={{ justifyContent: "space-between", alignItems: "flex-start", gap: 12, marginBottom: 6, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ marginBottom: 6 }}>DodoCard</h1>
          <p className="muted" style={{ marginBottom: 0, maxWidth: 560 }}>
      ...
      <div style={{ marginTop: 24 }}>
        {cards.length === 0 && <Empty text="Aucune DodoCard émise." />}
        ...
        <div className="grid cols-3">
          {cards.map((c: any) => (
            <div className="card" key={c.id}>
              ...
              <img
                src={api.qrUrl(c.id)}
                alt={`QR DodoCard ${c.patient_detail.full_name}`}
              ...
              {isAdmin && (
                <div className="row" style={{ gap: 8, marginTop: 14, justifyContent: "center", flexWrap: "wrap" }}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (window.confirm("Réémettre une nouvelle DodoCard ?")) {
                        reissue.mutate(c.id);
                      }
                    }}
                  >
